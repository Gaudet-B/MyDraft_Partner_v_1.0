...
class ScraperUtil:
    ...
    def scrape_and_parse_player_data(self):
    ...
    def scrape_and_parse_espn_projections(self):
        print('---------- beginning Selenium SCRAPE ----------')
        url = ESPN_URL
        self.driver.get(url)
        # let the page load
        ...
        toggle = self.driver.find_element(By.CLASS_NAME, 'ButtonGroup').find_element(By.CLASS_NAME, 'Button')
        ActionChains(self.driver).move_to_element(toggle).pause(1).click(toggle).pause(1).perform()
        ...
        for i in range(int(pages)):
            ...
            nav = self.driver.find_element(By.CLASS_NAME, NAV_CLASS)
            button = nav.find_element(By.CLASS_NAME, BUTTON_CLASS)
            
            if button.is_enabled():
                ActionChains(self.driver).move_to_element(button).pause(1).click(button).pause(1).perform()
...
# Does two things:
#  1. removes unnecessary cols: [ 'Percent', 'Change in Percent' ]
#  2. matches 'title' to a map that gets desired value (ex: { 'TD Pass': 'pass_td' })
